...
from utils import dist_util, logger, compute_fid
...
from sibm.sampling import sample_loop
import wandb
...
class TrainLoop:
    ...
    def run_loop(self):
        while True:
            for batch, cond, _ in self.data:
                ...
                # reverse
                if self.pred_mode.endswith("reverse"):
                    batch, cond = cond, batch
                ...
                # augmentation
                if self.augment is not None:
                    img_channels = batch.shape[1]
                    bc = th.concat([batch, cond['xT']], dim=1)
                    bc, _ = self.augment(bc)
                    batch = bc[:, :img_channels, :, :]
                    cond['xT'] = bc[:, img_channels:, :, :]
                ...
                if took_step and self.step % self.save_interval == 0:
                    ...
                    # reverse
                    if self.pred_mode.endswith("reverse"):
                        test_batch, test_cond = test_cond, test_batch
    ...
    def forward_backward(self, batch, cond, train=True):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            compute_losses = functools.partial(
                    ...
                    # self.diffusion.training_score_losses,
                    self.ddp_model,
                    micro,
                    t,
    ...
    def _anneal_lr(self):
        ...
        decay_rate = 0.95
        if self.step % self.lr_anneal_steps == 0:
            lr = self.lr * max(decay_rate ** frac_done, 0.01)
            print(f"lr: {lr}")
            # self.lr = lr
    ...
    def evaluate(self):
        print("evaluate: ...")
        eva_model = copy.deepcopy(self.model)
        params = self.mp_trainer.master_params
        state_dict = self.mp_trainer.master_params_to_state_dict(params)
        eva_model.load_state_dict(state_dict)
        eva_model.eval()
        
        dataloader = self.data if self.sample_kwargs.split == "train" else self.test_data
        print("Sampling ...")
        real_imgs, sample_imgs = sample_loop(diffusion = self.diffusion,
                                            model = eva_model,
                                            dataloader = dataloader,
                                            num_samples = self.sample_kwargs.num_samples,
                                            pred_mode = self.pred_mode,
                                            split = self.sample_kwargs.split,
                                            steps = self.sample_kwargs.steps,
                                            clip_denoised = self.sample_kwargs.clip_denoised,
                                            sampler = self.sample_kwargs.sampler,
                                            sigma_min = self.sample_kwargs.sigma_min,
                                            sigma_max = self.sample_kwargs.sigma_max,
                                            churn_step_ratio = self.sample_kwargs.churn_step_ratio,
                                            route_scaling = self.sample_kwargs.route_scaling,
                                            rho = self.sample_kwargs.rho,
                                            guidance = self.sample_kwargs.guidance)
        fid = compute_fid(real_imgs, sample_imgs, self.sample_kwargs.num_samples, self.sample_kwargs.batch_size, dist_util.dev())
